...
## 👑 Lead Developer — Variante 1

<div align="center">

| | Nombre | Carrera | Rol en esta variante |
|:---:|:---|:---|:---:|
| ⭐ | **Chambi Condori Janet** | **Ingeniería de Sistemas** | **`LEAD DEVELOPER`** |
...
| 👤 | Luis Fernando Quispe Sullca | Ingeniería en Ciencias de la Computación | Colaborador |

</div>

> 💡 **Nota:** Todos los integrantes contribuyeron por igual al desarrollo del laboratorio. El rol de *Lead Developer* en esta variante es asignado a **Janet Chambi Condori** como representante del flujo de trabajo colaborativo en Git.
...
network:
  ...
  ethernets:
    enp0s3:
      dhcp4: yes          # NAT — acceso SSH por Warp
    enp0s8:
      dhcp4: no
      addresses:
        - 192.168.237.100/24
...
```nginx
# Virtual host NGINX con PHP-FPM
server {
    listen 80;
    server_name _;
...
<!DOCTYPE html>
<html lang="es">
<head><meta charset="UTF-8"><title>Servidor <?=gethostname()?></title></head>
<body>
    <h1>🐘 PHP | Servidor: <?=gethostname()?> | IP: <?=$_SERVER['SERVER_ADDR']?></h1>
    <p>Tecnología: PHP-FPM <?=phpversion()?></p>
</body>
</html>
```
...
chmod +x /etc/local.d/nodejs.start
...
Las capturas documentan los siguientes puntos clave del laboratorio:
